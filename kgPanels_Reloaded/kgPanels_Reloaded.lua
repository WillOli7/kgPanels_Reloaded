...
function kgPanels:OnInitialize()
	...
	if DataBroker then
		local launcher = DataBroker:NewDataObject("kgPanels", {
		    type = "launcher",
		    icon = "Interface\\AddOns\\kgPanels_Reloaded\\icon.tga",
		    OnClick = function(clickedframe, button)
				local loaded = C_AddOns.IsAddOnLoaded("kgPanelsConfig_Reloaded")
				if loaded then
				  LibStub("AceConfigDialog-3.0"):Open("kgPanelsConfig")
				else
				  local ok, reason = C_AddOns.LoadAddOn("kgPanelsConfig_Reloaded")
				  if ok then
					LibStub("AceConfigDialog-3.0"):Open("kgPanelsConfig")
				  else
					kgPanels:Print(reason)
				  end
				end
		    end,
		})
...
function kgPanels:CommandLine(input)
	...
	if cmd == l_config then
		local ok, reason = C_AddOns.LoadAddOn("kgPanelsConfig_Reloaded")
		if ok then
			-- NOTE: we still open "kgPanelsConfig" because the options table is registered under that name
			LibStub("AceConfigDialog-3.0"):Open("kgPanelsConfig")
		else
			self:Print(reason or "Unable to load kgPanelsConfig_Reloaded.")
		end
		return
	end
